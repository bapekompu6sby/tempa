...
<div class="container mx-auto py-8">
    <div class="max-w-2xl mx-auto bg-white p-6 shadow rounded">
        <h2 class="text-2xl font-bold mb-6">Tambah Monitoring Template</h2>
        ...
        <form action="{{ route('monitoring-templates.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="category" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="sarpras" {{ old('category') == 'sarpras' ? 'selected' : '' }}>Sarpras</option>
                    <option value="administrasi" {{ old('category') == 'administrasi' ? 'selected' : '' }}>Administrasi</option>
                    <option value="administrasi_peserta" {{ old('category') == 'administrasi_peserta' ? 'selected' : '' }}>Administrasi Peserta</option>
                </select>
            </div>

            <div class="mb-4">
                <label class="block text-gray-700 mb-2 font-semibold">Sub Kategori</label>
                <input type="text" name="sub_category" class="w-full border rounded px-3 py-2" value="{{ old('sub_category') }}" placeholder="Opsional">
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="ownership" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="event" {{ old('ownership') == 'event' ? 'selected' : '' }}>Event</option>
                    <option value="event_subject" {{ old('ownership') == 'event_subject' ? 'selected' : '' }}>Event Subject</option>
                </select>
            </div>

            <div class="flex items-center gap-4">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded font-semibold hover:bg-blue-700">Simpan</button>
