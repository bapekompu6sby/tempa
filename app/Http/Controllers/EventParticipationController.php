<?php

namespace App\Http\Controllers;

use App\Models\EventParticipation;
use App\Http\Requests\StoreEventParticipationRequest;
use App\Http\Requests\UpdateEventParticipationRequest;

class EventParticipationController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreEventParticipationRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(EventParticipation $eventParticipation)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(EventParticipation $eventParticipation)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateEventParticipationRequest $request, EventParticipation $eventParticipation)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(EventParticipation $eventParticipation)
    {
        //
    }
}
